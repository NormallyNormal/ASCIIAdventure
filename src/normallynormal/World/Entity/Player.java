...
public class Player extends Entity implements GlowingEntity {
    ...
    private record State(double posX, double posY, int depth, boolean onScreen,
                         boolean dead, double timeDead) implements RenderState {
        @Override
        public void render(DepthScreen screen, int xOffset, int yOffset) {
            double rx = posX + 0.5;
            double ry = posY + 0.5;
            boolean half_x = rx % 1 > 0.75 || rx % 1 < 0.25;
            boolean half_y = ry % 1 > 0.75 || ry % 1 < 0.25;

            double round_x = Math.round(rx);
            double round_y = Math.round(ry);

            if (dead) {
                if (timeDead < 0.15)
                    screen.setCharacterWithDepth((int) rx, (int) ry, xOffset, yOffset, depth, CHAR_FULL);
                else if (timeDead < 0.3)
                    screen.setCharacterWithDepth((int) rx, (int) ry, xOffset, yOffset, depth, CHAR_DARK_SHADE);
                else if (timeDead < 0.45)
                    screen.setCharacterWithDepth((int) rx, (int) ry, xOffset, yOffset, depth, CHAR_MEDIUM_SHADE);
                else if (timeDead < 0.6)
                    screen.setCharacterWithDepth((int) rx, (int) ry, xOffset, yOffset, depth, CHAR_LIGHT_SHADE);
            } else {
                if (!half_x && !half_y) {
                    screen.setCharacterWithDepth((int) rx, (int) ry, xOffset, yOffset, depth, CHAR_FULL);
                } else if (half_x && !half_y) {
                    screen.setCharacterWithDepth((int) round_x - 1, (int) ry, xOffset, yOffset, depth, CHAR_RIGHT_HALF);
                    screen.setCharacterWithDepth((int) round_x, (int) ry, xOffset, yOffset, depth, CHAR_LEFT_HALF);
                } else if (!half_x) {
                    screen.setCharacterWithDepth((int) rx, (int) round_y - 1, xOffset, yOffset, depth, CHAR_LOWER_HALF);
                    screen.setCharacterWithDepth((int) rx, (int) round_y, xOffset, yOffset, depth, CHAR_UPPER_HALF);
                } else {
    ...
    public void copyForRender() {
        renderState = new State(position.x, position.y, depth, isOnScreen(), dead, timeDead);
    }
