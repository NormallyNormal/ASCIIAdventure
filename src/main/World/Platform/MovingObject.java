...
public class MovingObject extends StaticObject {
        ...
        if (xSteps > 0) {
            if(movesNextFrame) {
                if (entity.isGravityDownward()) {
                    ...
                    if (entity.getCollisionBox().y >= collisionBox.y + collisionBox.h - 0.1) {
                        dragEntityHorizontally(entity);
                    }
                }
            }
    ...
    private void pushEntityHorizontally(Entity entity) {
        ...
        switch (xDirection) {
            case LEFT:
                if (entity.getPosition().x <= collisionBox.x + 0.1) {
                    newPos = Math.nextDown(collisionBox.x - entity.getCollisionBox().w);
                    apply = true;
                }
                break;
            case RIGHT:
                if (entity.getPosition().x >= collisionBox.x + collisionBox.w - 1.1) {
